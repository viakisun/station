"use client";
/* ============================================================
   앱 셸 — TopBar / LeftNav / EventStrip / ConsoleShell
   (원본: farm-control/shell.jsx — 마크업·스타일 1:1 보존,
    window 전역 → props/context, 라우팅은 콜백으로 위임)
   ============================================================ */
  ...
  return (
    ...
    >
      ...
      >
        ...
        <input
          placeholder="Search robots · work · incidents · modules"
          style={{ border: "none", outline: "none", background: "transparent", flex: 1, fontSize: 13, color: "var(--ink)", fontFamily: "inherit" }}
        />
      ...
      <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
        ...
        <StatPill
          ...
          value={kpi.incidentsOpen}
          sev={kpi.incidentsCritical ? "critical" : "disabled"}
          onClick={() => onNavWorkspace("incident")}
        />
...
function StatPill({
  ...
  return (
    ...
    >
      ...
      <span className="tnum" style={{ fontSize: 13, fontWeight: 800, color: alert ? "var(--st-critical)" : "var(--ink)" }}>
        {value}
      </span>
    </button>
  );
...
export function EventStrip({
  ...
  const meta = CONTROL.sevMeta[cur.sev]!;
  ...
  return (
    <div style={{ flex: "none", background: "var(--surface)", borderTop: "1px solid var(--line)", zIndex: 35 }}>
      ...
      <div style={{ height: "var(--eventstrip-h)", display: "flex", alignItems: "center", gap: 12, padding: "0 14px" }}>
        ...
        <button
          onClick={() => onSelectRobot && cur.src.startsWith("RBT") && onSelectRobot(cur.src)}
        ...
        <span style={{ fontSize: 11, color: "var(--ink-3)", flex: "none" }} className="tnum">
          {idx + 1}/{events.length}
        </span>
      ...
      {expanded && (
        <div style={{ maxHeight: 220, overflow: "auto", borderTop: "1px solid var(--line)", background: "var(--surface-2)" }}>
          {events.map((e, i) => {
            const m = CONTROL.sevMeta[e.sev]!;
            return (
              <div
                key={i}
                onClick={() => e.src.startsWith("RBT") && onSelectRobot && onSelectRobot(e.src)}
                style={{
                  display: "flex",
                  alignItems: "center",
                  gap: 12,
                  padding: "8px 16px",
                  borderBottom: "1px solid var(--line)",
                  cursor: e.src.startsWith("RBT") ? "pointer" : "default",
                  background: "var(--surface)",
                }}
              >
                <span className="mono" style={{ fontSize: 11, color: "var(--ink-3)", width: 64, flex: "none" }}>
                  {e.t}
                </span>
                <StatusBadge sev={m.sev} label={m.label} />
                <span className="mono" style={{ fontSize: 11.5, color: "var(--ink-2)", width: 120, flex: "none" }}>
                  {e.src}
                </span>
...
/* ============================================================
   ConsoleShell — 데스크톱 통합 셸 (TopBar+LeftNav+EventStrip+RobotDrawer)
   라우팅은 onNav/onOpenSession 콜백으로 앱에 위임. 로봇 드로어 상태는
   내부 보관 + ShellContext 로 화면에 selectRobot/openSession 제공.
   ============================================================ */
export function ConsoleShell({
  ...
  active: string;
  onNav: (id: string, ctx?: Record<string, unknown>) => void;
  ...
  const api = {
    selectRobot: (id: string) => setRobotId(id),
    openSession: (id: string) => {
      setRobotId(null);
      onOpenSession ? onOpenSession(id) : onNav("control");
    },
    navWorkspace: (id: string, ctx?: Record<string, unknown>) => {
      setRobotId(null);
      onNav(id, ctx);
    },
  };
  ...
  return (
    <ShellContext.Provider value={api}>
      <div style={{ height: "100vh", display: "flex", flexDirection: "column", background: "var(--canvas)" }}>
        <TopBar kpi={CONTROL.kpi} site={CONTROL.SITE} onNavWorkspace={api.navWorkspace} />
        <div style={{ flex: 1, display: "flex", minHeight: 0 }}>
          <LeftNav active={active} onNav={onNav} collapsed={navCollapsed} onToggle={() => setNavCollapsed((v) => !v)} />
          <main data-theme={theme} style={{ flex: 1, minWidth: 0, overflow: "auto", display: "flex", flexDirection: "column" }}>{children}</main>
        </div>
        <EventStrip expanded={stripOpen} onToggle={() => setStripOpen((v) => !v)} onSelectRobot={api.selectRobot} />
      </div>
      {robotId && (
        <RobotDrawer
          robotId={robotId}
          onClose={() => setRobotId(null)}
          onOpenSession={api.openSession}
          onNavWorkspace={api.navWorkspace}
        />
      )}
