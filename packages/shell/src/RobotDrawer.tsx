...
/* ============================================================
   ...
   (원본: farm-control/drawer.jsx — 마크업·스타일 1:1 보존)
   ============================================================ */
...
export function RobotDrawer({
  ...
  onNavWorkspace?: (id: string, ctx?: Record<string, unknown>) => void;
}) {
  const M = CONTROL;
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        {session && (
          <MiniCard
            ...
            onClick={() => onOpenSession && onOpenSession(session.id)}
            icon="ext"
          >
            <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 7 }}>
              ...
              <StatusBadge
                sev={M.workMeta[session.status]!.sev}
                label={M.workMeta[session.status]!.label}
              />
            </div>
    ...
    Firmware: (
      <div style={{ display: "flex", flexDirection: "column", gap: 9 }}>
        {mods.map((m) => (
          <div
            key={m.id}
            style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "9px 0", borderBottom: "1px solid var(--line)" }}
          >
            <div>
            ...
            <span
              className="mono"
              style={{ fontSize: 12, fontWeight: 700, color: m.id === "MOD-CAM-V01-0019" ? "var(--st-warning)" : "var(--ink)" }}
            >
              v{m.fw}
              {m.id.startsWith("MOD-CAM") ? " → 2.4.2" : ""}
            </span>
          </div>
        ))}
        <button className="btn" style={{ width: "100%" }} onClick={() => onNavWorkspace && onNavWorkspace("firmware", { origin: "ops", robot_id: robot.id, module_id: mods[0]?.id })}>
        ...
        <KV
          pairs={[
            ...
            [
              "Connection",
              <StatusBadge
                sev={robot.state === "offline" ? "disabled" : "normal"}
                label={robot.state === "offline" ? "disconnected" : "connected"}
              />,
            ],
            ["Resolution", "1024 × 600"],
          ]}
        />
        <button className="btn" style={{ width: "100%" }} onClick={() => onNavWorkspace && onNavWorkspace("hmi", { origin: "ops", robot_id: robot.id, work_session_id: robot.sessionId ?? undefined })}>
        ...
        <KV
          pairs={[
            ...
            [
              ...
              <StatusBadge
                sev={robot.state === "offline" ? "disabled" : "normal"}
                label={robot.state === "offline" ? "offline" : "good"}
              />,
            ],
            ["Sync", robot.state === "fault" ? "lag 1.2s" : "realtime"],
          ]}
        />
        <button className="btn" style={{ width: "100%" }} onClick={() => onNavWorkspace && onNavWorkspace("telemetry", { origin: "ops", robot_id: robot.id })}>
  ...
  return (
    <>
      <div
        onMouseDown={onClose}
        style={{ position: "fixed", inset: 0, background: "rgba(20,25,35,.18)", zIndex: 90 }}
      />
      <aside
        style={{
          position: "fixed",
      ...
      >
        ...
        <div style={{ display: "flex", gap: 7, padding: "12px 18px", borderBottom: "1px solid var(--line)" }}>
          {isEmergency ? (
            ...
            <>
              {canResume ? (
                <button className="btn primary" style={{ flex: 1 }} onClick={() => setConfirm("resume")}>
                  <Icon name="play" size={14} /> Resume
                </button>
              ) : (
                <button className="btn" style={{ flex: 1 }} disabled={!canPause} onClick={() => setConfirm("pause")}>
                  <Icon name="pause" size={14} /> Pause
                </button>
              )}
              <button className="btn" style={{ flex: 1 }} onClick={() => setConfirm("return")}>
                <Icon name="home2" size={14} /> Return
              </button>
            </>
          )}
        </div>
          ...
          <>
            <div style={{ display: "flex", gap: 2, padding: "8px 12px 0", overflowX: "auto", borderBottom: "1px solid var(--line)" }}>
              {TABS.map((t) => (
                ...
                >
                  {t}
                  {t === "Incidents" && incs.length ? ` ${incs.length}` : ""}
                </button>
              ))}
            </div>
        ...
        >
          ...
          <button
            style={{ border: "none", background: "transparent", color: "var(--brand)", fontWeight: 700, fontSize: 11.5, padding: 0, marginLeft: 2 }}
          >
            history
          </button>
        </div>
      ...
      <ConfirmModal
        open={confirm === "pause"}
        danger={false}
        title="Pause work request"
        requireHold
        body={
          <>
            Pause the running work on <b>{robot.id}</b>. The robot will stop at a safe position.
          </>
        }
      ...
      <ConfirmModal
        open={confirm === "resume"}
        title="Resume work request"
        body={
          <>
            Resume work on <b>{robot.id}</b> after confirming safety state and route occupancy.
          </>
        }
        confirmLabel="Resume"
        onConfirm={() => setConfirm(null)}
      ...
      <ConfirmModal
        open={confirm === "return"}
        title="Return-to-dock request"
        body={
          <>
            Return <b>{robot.id}</b> to its charge / dock position.
          </>
        }
        confirmLabel="Return"
        onConfirm={() => setConfirm(null)}
      ...
      <ConfirmModal
        ...
        body={
          <>
            Releasing the e-stop requires all safety conditions to be met. Have you confirmed on-site safety yourself?
          </>
        }
...
export function MiniCard({
  ...
  children: ReactNode;
  onClick?: () => void;
  icon?: import("@station/design-system").IconName;
}) {
  return (
