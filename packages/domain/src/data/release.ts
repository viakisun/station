/* ============================================================
   verify·배포 콘솔 — Mock 데이터 (C02 Audit / C04 Firmware)
   제조사 이기종 모듈 표준화(Audit) + 펌웨어/OTA 배포
   (원본: release-console/data.js · window.RC — 값 1:1 보존)
   ============================================================ */
...
const auditMeta: MetaMap = {
  draft: { label: "draft", sev: "disabled" },
  submitted: { label: "submitted", sev: "notice" },
  running: { label: "running", sev: "notice" },
  passed: { label: "passed", sev: "normal" },
  failed: { label: "failed", sev: "critical" },
  waiver_required: { label: "waiver required", sev: "warning" },
  approved: { label: "approved", sev: "normal" },
  expired: { label: "expired", sev: "disabled" },
};
...
const sevMeta: MetaMap = {
  ...
  low: { label: "low", sev: "notice" },
  warning: { label: "warning", sev: "warning" },
  critical: { label: "critical", sev: "critical" },
  blocker: { label: "blocker", sev: "emergency" },
};
...
] as { id: string; suite: string; name: string; ms: number; fail?: boolean; exp?: string; act?: string }[];

// C04 firmware
const fwMeta: MetaMap = {
  draft: { label: "draft", sev: "disabled" },
  analyzing: { label: "analyzing", sev: "notice" },
  blocked: { label: "blocked", sev: "critical" },
  approved: { label: "approved", sev: "normal" },
  scheduled: { label: "scheduled", sev: "notice" },
  deploying: { label: "deploying", sev: "notice" },
  success: { label: "success", sev: "normal" },
  failed: { label: "failed", sev: "critical" },
  rollback_required: { label: "rollback required", sev: "emergency" },
  rolled_back: { label: "rolled back", sev: "warning" },
};
const fwAnalysisMeta: MetaMap = {
  passed: { label: "passed", sev: "normal" },
  warning: { label: "warning", sev: "warning" },
  blocked: { label: "blocker", sev: "critical" },
  analyzing: { label: "analyzing", sev: "notice" },
  failed: { label: "failed", sev: "critical" },
};
const firmwares = [
...
] as { id: string; sev: string; rule: string; title: string; file: string; fn: string; waiver: string | null }[];
...
const compatMeta: Record<string, { label: string; sev: string; mark: string }> = {
  ok: { label: "compatible", sev: "normal", mark: "✓" },
  warn: { label: "warning", sev: "warning", mark: "!" },
  incompat: { label: "incompatible", sev: "critical", mark: "✕" },
  unknown: { label: "unknown", sev: "disabled", mark: "?" },
...
] as { id: string; gh: string; group: string; stage: number; fail?: boolean }[];
...
export type ReleaseData = typeof RELEASE;
