...
## 1. 단일 진실 공급원 (SSOT)
- **★ [01. 통합 오케스트레이션 — 핵심 가치 모델](01-integration-orchestration.md)** ([ADR-022](adr/ADR-022-integration-orchestration-core-value.md)) — STATION = ① 통합 미들웨어(Local Agent/RAL) + ② 통합 오케스트레이션(readiness·할당·릴리스). **STATION이 무엇이고 누구를 위한 것인지의 정의.** Landing([apps/hub](../apps/hub/app/page.tsx))이 렌더하는 모델.
...
- **★ STATION SDV Reference Platform — Technical Baseline (통합 기술 기준서, SSOT)** — **[HTML](architecture/sdv-reference-platform.html) / [PDF(A3, 53p)](architecture/sdv-reference-platform.pdf)** · 문서번호 `STATION-SDV-SPEC-2026-001` Rev A. 생육분석 로봇 SDV의 **단일 기준서**: Front matter(Scope·Org registry·ID 규약) + **Part A 소프트웨어 아키텍처** + **Part B STATION SDV Identifier System**(Master Prefix Registry(전 식별자 ~24 family)·Namespace Map·Naming&Governance·10-레이어·정본 레지스트리·Deployment Matrix·Asset-tag) + **Part C 프로토콜·메시지**(2-tier transport·ProtocolProfile registry·transport matrix·pub/sub·3단계 ACK·Gate·교환 시퀀스) + **Part D 리그**(RIG-1 평면 패널 · **RIG-2 프로파일 로버** · 공통 전원/E-stop) + **Part E 운용**(run loop·HMI·OTA) + **Part F Module Interfaces (ICD)**(3-class IF-P/IF-L/IF-X·인터페이스 맵·**16 인터페이스 풀 시트**·provider/consumer 매트릭스·heartbeat·클라우드 토픽 트리·명령 인가 경로) + **Part G Software-Defined Work Layer**(5 레이어·Application(1단위 2실행면)·AppManifest·`station.app.*`·App Runtime 생명주기·Command Governance·작업 매핑·**벤더 번들 채용**·**멀티벤더 conformance(노드·모듈·앱·HMI·베이스)**) + **Part H Software Work-Breakdown & Commercial SaaS Structure**(SW-WBS 식별체계 SWC/SWS/SWT·상업 SaaS Product IA(Ops·Build·Field·Agent)·on-robot↔cloud 미러·Surface/Component/Task 레지스트리·Definition of Done — [ADR-019](adr/ADR-019-commercial-saas-scoping-and-sw-wbs.md)) + Annex A–H(계약 JSON·ID 문법·도면 색인·**코드 정합 변경안**·요구사항 레지스터·도표 목록·변경이력). 2색(ink+안전 적색)·드라이 용어. 본 문서 = **제3자 채택용 오픈 SDV 레퍼런스 표준 + 레퍼런스 구현**([ADR-017](adr/ADR-017-open-source-sdv-reference.md)). **아래 3개 문서를 통합·대체**(deprecated). *(UI 체계 정리 = 추후 별도 트랙.)*
...
**010 Contracts SSOT(JSON Schema)** · **011 노드/기관 소유 모델** · **012 CommandEnvelope+3단계 ACK** · **013 신호 NS(machine.*/env.*)** · **014 Robot Blueprint·개방형 NodeKind·platform/profile 분리** · **015 첫 구현 단위=Reference Local Agent(HMI는 하류)** · **016 Software-Defined Work Layer(Application·App Runtime·모듈 교체 로딩)** · **017 Open-source SDV Reference & Standardization** · **018 Contracts SSOT 정합(동결 해제: 과도 enum 개방·stale 보정·AppManifest/panel-host 신설)** · **019 Commercial SaaS Scoping & SW-WBS(SWC/SWS/SWT 식별체계·on-robot↔cloud 미러·ORG→PRJ→SITE→RBT·CommandEnvelope 불변·ADR-001 supersede)** · **020 Industrial SDV Token System(다크·4계층·product/authority/locus/mirror/state·density·전면 rename)** · **021 중립 제품 정체성·안전색 잠금·authority 명도 rank(제품 색 0·green/amber/red 상태 전용·② >③>④ 명도 위계·locus 아이콘·mirror 중립·ADR-020 색 결정 일부 정정)**.
