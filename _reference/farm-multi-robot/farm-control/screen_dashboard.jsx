/* ============================================================
   C01-00 통합 운영 대시보드
   KPI 6 · 온실 상태 카드 · 진행 작업 테이블 · critical 이벤트
   · 로봇 유형별 가동률 · 빠른 필터
   layout variant (Tweak): "balanced" | "ops-first"
   ============================================================ */

function DashboardScreen({ onSelectRobot, onOpenSession, onNavScreen, layout = "balanced", density }) {
  const M = window.MOCK;
  ...
  const util = (arr) => Math.round(arr.filter(r => r.state === "working").length / arr.length * 100);
          ...
          return (
            <div key={g.id} style={{ border: "1px solid var(--line)", borderRadius: "var(--r-md)", padding: 13 }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 9 }}>
                ...
                <StatusBadge sev={g.state} label={g.state === "warning" ? "env warning" : "normal"} dot={false} />
              </div>
...
function EnvStat({ label, val, unit, warn }) {
  return (
...
function StateDistribution({ robots }) {
  const M = window.MOCK;
  const counts = {};
...
Object.assign(window, { DashboardScreen, Chip });
