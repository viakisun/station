...
/* ============================================================
   ...
   (원본: telemetry-console/screens_monitor.jsx)
   ============================================================ */
import { useEffect, useState, type ReactNode } from "react";
import { Icon, StatusBadge, KpiCard, Sparkline, PanelHead, type Sev } from "@station/design-system";
import { TELEMETRY } from "@station/domain";
import { TouchBtn } from "./helpers";
    ...
    <div style={{ flex: 1, minHeight: 0, display: "flex", flexDirection: "column" }}>
      <div style={{ flex: "none", display: "flex", gap: 4, padding: "10px 18px 0", borderBottom: "1px solid var(--line)" }}>
        {[["overview", "Overview"], ["quality", "Data quality"]].map(([k, l]) => {
          const on = tab === k;
          return (
            <button key={k} onClick={() => setTab(k)} style={{ padding: "9px 14px", border: "none", background: "transparent", fontSize: 13.5, fontWeight: on ? 700 : 600, color: on ? "var(--ink)" : "var(--ink-3)", borderBottom: "2px solid " + (on ? "var(--ink)" : "transparent"), marginBottom: -1 }}>{l}</button>
          );
        })}
      </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1.5fr 1fr", gap: 14, alignItems: "start" }}>
        <div className="card">
          <PanelHead
            ...
            sub={`${T.quality.length} active channels`}
            dense
            right={
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
          ...
          <div className="card">
            ...
            <div style={{ padding: 10, display: "flex", flexDirection: "column", gap: 7 }}>
              {([["08:50", "warning", "TCH-gh-humidity nearing threshold 74%"], ["08:44", "info", "TCH-arm-joint-temp calibration applied"], ["08:38", "warning", "TCH-cam-fps data gap 12%"]] as [string, string, string][]).map((e, i) => (
    ...
    <div style={{ padding: 18, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14, height: "100%", minHeight: 0 }}>
      ...
      <div className="card" style={{ display: "flex", flexDirection: "column", minHeight: 0 }}>
        ...
        <div style={{ flex: 1, overflow: "auto" }}>
          {T.quality.map((q) => {
            const qm = T.qualityMeta[q.q],
              on = sel === q.ch;
            return (
              <button key={q.ch} onClick={() => setSel(q.ch)} style={{ width: "100%", textAlign: "left", display: "flex", alignItems: "center", gap: 11, padding: "11px 14px", border: "none", borderBottom: "1px solid var(--line)", borderLeft: on ? "2px solid var(--ink)" : "2px solid transparent", background: on ? "var(--surface-2)" : "transparent" }}>
      ...
      {/* detail */}
      <div className="card" style={{ display: "flex", flexDirection: "column", minHeight: 0 }}>
        <PanelHead title={cur.label + " diagnostics"} sub={cur.ch} dense right={cur.q === "bad" ? <span className="live-pulse" style={{ fontSize: 10.5, color: "var(--st-critical)", fontWeight: 700 }}>● anomaly</span> : undefined} />
...
function Metric({ label, v, warn }: { label: string; v: ReactNode; warn?: boolean }) {
  return (
