...
export default function Page() {
  ...
  return (
    <div>
      <SurfaceHeader sws="SWS-FIELD-HMI" right={<span className="mono" style={{ fontSize: 11, color: "var(--ink-3)" }}>Field HMI Mock — on-robot operator panel</span>} />
      <div style={{ padding: "var(--pad-card)", display: "flex", flexDirection: "column", gap: "var(--gap)" }}>
        <div className="card" style={{ padding: "var(--pad-card)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 12, color: "var(--ink-2)" }}>authority ③ · local-first · 현장 직접 조작</div>
            </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "var(--gap)" }}>
          {CONTROLS.map((c) => (
            <button key={c.label} className={`btn ${c.primary ? "primary" : ""}`} style={{ height: "var(--control-h)", fontSize: 16, gap: 8 }} disabled={!scope.robotId}>
              <Icon name={c.icon} size={22} /> {c.label}
            </button>
          ))}
        </div>
        <div className="card" style={{ padding: "var(--pad-card)", fontSize: 13, color: "var(--ink-2)", lineHeight: 1.6 }}>
