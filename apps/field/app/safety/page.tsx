...
export default function Page() {
  ...
  return (
    <div>
      <SurfaceHeader sws="SWS-FIELD-SAFETY" right={<span className="mono" style={{ fontSize: 11, color: "var(--text-muted)" }}>on-robot operator panel</span>} />
      <div style={{ padding: "var(--panel-padding)", display: "flex", flexDirection: "column", gap: "var(--panel-gap)" }}>
        <div className="card" style={{ padding: "var(--panel-padding)", borderLeft: "4px solid var(--state-emergency)" }}>
          <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
            <Icon name="power" size={24} style={{ color: "var(--state-emergency)" }} />
            <strong style={{ fontSize: 16 }}>긴급정지 (E-stop)</strong>
            <span style={{ marginLeft: "auto", fontSize: 11, color: "var(--text-muted)" }}>{scope.robotId ?? "로봇 미선택"}</span>
          </div>
          <HoldButton label="E-STOP 발동 (hold)" holdLabel="hold…" danger duration={900} style={{ width: "100%", height: "var(--control-height)", fontSize: 16 }} />
          <div style={{ fontSize: 12, color: "var(--state-critical)", marginTop: 10 }}>
        ...
        <div className="card" style={{ padding: "var(--panel-padding)" }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 6, marginTop: 10 }}>
            {INTERLOCKS.map((it) => (
              <div key={it.label} style={{ display: "flex", alignItems: "center", gap: 10, padding: "6px 0", borderBottom: "1px solid var(--line-default)" }}>
