          ...
          <div key={p.id} className="card" style={{ padding: 12 }}>
            ...
            {getSitesOfProject(p.id).map((s) => (
              <div key={s.id} style={{ marginTop: 6 }}>
                <div className="mono" style={{ fontSize: 11, color: "var(--text-secondary)", marginBottom: 4 }}>{s.id} · {s.name}</div>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
                  {getRobotsOfSite(s.id).map((r) => (
                    <button key={r.id} className="btn sm" onClick={() => { setProject(p.id); setRobot(r.id); }}
                      style={{ height: 28, borderColor: scope.robotId === r.id ? "var(--text-primary)" : undefined }}>
                      <span className="mono" style={{ fontSize: 10.5 }}>{r.id}</span>
                      <span style={{ fontSize: 9.5, color: "var(--text-secondary)", marginLeft: 5 }}>{r.type}</span>
                      <span style={{ fontSize: 9.5, color: "var(--text-muted)", marginLeft: 5 }}>{r.state} · {r.battery}%</span>
                    </button>
                  ))}
                </div>
              </div>
