...
import { CONTROL, type Robot } from "@station/domain";
import {
  Icon,
  StatusBadge,
  RobotTypeTag,
...
import { Chip } from "./DashboardScreen";

interface RealtimeMapScreenProps {
  selectedRobot?: string | null;
  mapStyle?: "schematic" | "tinted";
  density?: "regular" | "compact";
}

export function RealtimeMapScreen({ selectedRobot = null, mapStyle = "schematic", density }: RealtimeMapScreenProps) {
  const M = CONTROL;
  const { selectRobot, openSession, navWorkspace } = useShell();
  const onSelectRobot = (id: string) => selectRobot(id);
  const onOpenSession = (id: string) => openSession(id);
  const onNavWorkspace = (id: string) => navWorkspace(id);

  const [gh, setGh] = useState("GH-A");
  const [typeFilter, setTypeFilter] = useState("ALL");
  const [tick, setTick] = useState(0);
  const [lockZone, setLockZone] = useState<{ x: number; y: number } | null>(null);
  ...
  const jitter = (r: Robot, axis: string) => {
