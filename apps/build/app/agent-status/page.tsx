      ...
      <div style={{ padding: 14, display: "grid", gridTemplateColumns: "1.3fr 1fr", gap: 14 }}>
        <div className="card" style={{ padding: 12 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 4, marginTop: 8 }}>
            {NODES.map((n) => (
              <div key={n.kind} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 11.5, padding: "4px 0", borderBottom: "1px solid var(--line-default)" }}>
                <span className="mono" style={{ width: 80, fontWeight: 700 }}>{n.kind}</span>
                <span style={{ flex: 1, color: "var(--text-secondary)" }}>{n.role}</span>
                <span className="mono" style={{ fontSize: 10, color: "var(--text-muted)" }}>{n.org} · {n.proto}</span>
              </div>
            ))}
          </div>
        </div>
        <div className="card" style={{ padding: 12 }}>
          ...
          <div style={{ fontSize: 12, color: "var(--text-muted)", lineHeight: 1.6, marginTop: 8 }}>
            Agent는 사용자 제품이 아니라 <b style={{ color: "var(--text-primary)" }}>on-robot daemon</b>(권한 ②) — command routing·policy·node registry. 깊은 라이브 인스펙션은 <b style={{ color: "var(--text-primary)" }}>Runtime Inspector</b>(/agent)에서.
          </div>
