    ...
    <div>
      ...
      <div style={{ padding: 14, display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 3, marginTop: 8 }}>
            {artifacts.map((a) => (
              <div key={a.f} style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 11.5, padding: "3px 0" }}>
                <Icon name={a.ok ? "check" : "alert"} size={13} style={{ color: a.ok ? "var(--state-normal)" : "var(--state-critical)" }} />
                <span className="mono" style={{ flex: 1 }}>{a.f}</span>
                <span style={{ fontSize: 10, color: "var(--text-muted)" }}>{a.note}</span>
              </div>
            ))}
          </div>
        </div>
        <div className="card" style={{ padding: 12 }}>
          ...
          <div className="mono" style={{ fontSize: 10, color: "var(--text-muted)", marginTop: 10 }}>suite 실행·리포트·waiver 게이트는 후속(SWT).</div>
        </div>
      </div>
    </div>
  );
